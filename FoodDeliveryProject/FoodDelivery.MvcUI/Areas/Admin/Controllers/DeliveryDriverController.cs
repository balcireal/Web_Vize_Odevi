    ...
    public class DeliveryDriverController : Controller
    {
        public async Task<IActionResult> Index()
        {
            using (HttpClient client = new HttpClient())
            {
                ...
                var httpResponseMessage = await client.GetAsync($"{client.BaseAddress}/deliverydrivers");

                var jsonResponse = await httpResponseMessage.Content.ReadAsStringAsync();

                var response =
                  JsonSerializer.Deserialize<ResponseBody<List<DeliveryDriverItem>>>(jsonResponse,
